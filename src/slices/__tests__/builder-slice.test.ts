...
describe('builderSlice', () => {
  ...
  describe('addBunBuilder', () => {
    it('Добавить булочку в конструктор', () => {
      ...
      expect(builderSlice(previousState, setBun(mockBun))).toEqual({
        constructorItems: {
          bun: mockBun,
          ingredients: []
        }
    ...
    it('Заменить существующую булочку при добавлении новой', () => {
      ...
      expect(builderSlice(previousState, setBun(newBun))).toEqual({
        constructorItems: {
          bun: newBun,
          ingredients: []
        }
    ...
    it('Установить значение bun равным null, когда полезная нагрузка равна null', () => {
      ...
      expect(builderSlice(previousState, setBun(null))).toEqual({
        constructorItems: {
          bun: null,
          ingredients: []
        }
  ...
  describe('moveItems', () => {
    ...
    it('Переместить элемент вверх', () => {
      ...
      const action = reorderIngredient({
        index: 1,
        direction: 'up'
      });
  ...
  describe('clearBuilder', () => {
    it('Очистить все элементы от конструктора', () => {
      ...
      const result = builderSlice(previousState, resetConstructor());
  ...
  describe('selectors', () => {
    ...
    it('selectConstructorTotalCount должен возвращать количество ингредиентов', () =>
      // @ts-ignore
      expect(selectIngredientsCount(state)).toBe(2));
  });
});
